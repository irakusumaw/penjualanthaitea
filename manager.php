<!DOCTYPE html>
<html>
<head>
  <title>Thai Tone's Cafe</title>
  <link rel="stylesheet" type="text/css" href="style.css">
</head>
<body>
  <div class="wrap">
    <div class="header">      
      <h1><font color="orange" align="center" face="Curlz MT" size="20">THAI TONE'S CAFE</font></h1>
      <p><font color="orange">Tempat Nongkrong Paling Hitz</font></p>
      <ul class="button"><a href="login.php" style="text-decoration: none"><font color="orange">ADMIN</font></a></ul>
    </div>

    <div class="badan">     
      <div class="sidebar">
      <br>
      <nav>
        <center>
      <ul class="button"><a href="lihatlaporan.php">Lihat Data Laporan</a></ul></br>
    </center>
    </nav>
      </div>
      <div class="content">
        <center>
        <img src="manager.png" style="width:300px; height:350px;"></img>
        <br></br>
        <font face="bizagi font" size="">SELAMAT DATANG DI MENU MANAGER</font>
      </center>
      </div>
    </div>
    <div class="clear"></div>
    <div class="footer">
      <font >Created By . Funny F Tampubolon & Ira K Wardani</font>
    </div>
  </div>
</body>
</html>